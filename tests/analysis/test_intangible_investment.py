"""Tests for the intangible_investment module."""
import numpy as np
import pandas as pd
import pytest
...
from tests.analysis.mocks.mock import (
    mock_capital_national_accounts,
    mock_growth_accounts,
)
...
def capital_accounts_indexed(capital_accounts):
    ...
    return capital_accounts.set_index(["country_code", "year"])
...
def national_accounts_indexed(national_accounts):
    ...
    return national_accounts.set_index(["country_code", "year"])
...
@pytest.mark.parametrize(
    "capital_accounts",
    [pd.DataFrame(), pd.DataFrame(columns=["a", "b", "c"])],
)
def test_get_share_of_intangible_investment_per_gdp_capital_accounts_invalid_columns(
    capital_accounts,
    national_accounts,
):
...
def test_get_share_of_intangible_investment_per_gdp_capital_accounts_wrong_type(
    capital_accounts,
    national_accounts,
):
...
def test_get_share_of_intangible_investment_per_gdp_national_accounts_invalid_columns(
    national_accounts,
    capital_accounts,
):
...
def test_get_share_of_intangible_investment_per_gdp_national_accounts_wrong_type(
    national_accounts,
    capital_accounts,
):
...
def test_get_share_of_intangible_investment_per_gdp_issue_warning(
    capital_accounts,
    national_accounts,
):
    ...
    with pytest.warns(
        UserWarning,
        match="Share of intangible investment is greater than 100% of GDP",
    ):
        get_share_of_intangible_investment_per_gdp(
            capital_accounts=capital_accounts,
            national_accounts=national_accounts,
...
def test_get_share_of_intangible_investment_per_gdp_investment_level(
    capital_accounts,
    national_accounts,
):
...
def test_get_share_of_intangible_investment_per_gdp_share_intangible_share_intangible_positive(
    capital_accounts,
    national_accounts,
):
    result = get_share_of_intangible_investment_per_gdp(
...
def test_get_composition_of_value_added_country_code_invalid(
    country_code,
    growth_accounts,
):
...
def test_get_intangible_investment_aggregate_types_raise_index_not_equal(
    capital_accounts_indexed,
    national_accounts,
):
    national_accounts["country_name"] = "BE"
    national_accounts = national_accounts.set_index("country_name")
...
def test_get_intangible_investment_aggregate_types_result(
    capital_accounts_indexed,
    national_accounts_indexed,
):
...
def test_get_share_of_tangible_investment_per_gdp_index_not_equal(
    capital_accounts_indexed,
    national_accounts,
):
    national_accounts["country_name"] = "BE"
...
def test_get_share_of_tangible_investment_per_gdp_result_column(
    capital_accounts_indexed,
    national_accounts_indexed,
):
...
def test_get_share_of_tangible_investment_per_gdp_result_index_match(
    capital_accounts_indexed,
    national_accounts_indexed,
):
    actual = get_share_of_tangible_investment_per_gdp(
