"""Function for mocking EU KLEMS cleaned data accounts."""
import pandas as pd
...
def mock_capital_national_accounts() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    category_dict = {
        category: RNG_FOR_TESTING.uniform(100, 2000, 5).tolist()
        for category in capital_accounts_columns
    }
...
def mock_growth_accounts() -> pd.DataFrame:
    category_dict = {
        category: RNG_FOR_TESTING.uniform(0.1, 1.0, 5).tolist()
        for category in LABOUR_COMPOSITION_COLUMNS
    }
    growth_accounts = pd.DataFrame(category_dict)
    growth_accounts["labour_productivity"] = RNG_FOR_TESTING.uniform(
        1.5,
        5.0,
        5,
    ).tolist()
