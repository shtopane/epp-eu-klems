...
def gdp_data(data_info):
    return clean_gdp_data.read_data(data_info=data_info, path=GDP_FIXTURE_PATH)


def test_read_data_return_type(data_info):
    gdp = clean_gdp_data.read_data(data_info=data_info, path=GDP_FIXTURE_PATH)
...
def test_read_data_invalid_data_info(data_info):
    with pytest.raises(TypeError, match="The data_info"):
        clean_gdp_data.read_data(data_info=data_info, path=GDP_FIXTURE_PATH)
...
def test_read_data_invalid_path(path, data_info):
    with pytest.raises(ValueError, match="<class 'pathlib.Path'>"):
        clean_gdp_data.read_data(data_info=data_info, path=path)
...
def test_clean_gdp_per_capita_columns(gdp_data, data_info):
    ...
    assert gdp_data_clean.reset_index().columns.tolist() == [
        "country_code",
        "year",
        "gdp_per_capita",
    ]
