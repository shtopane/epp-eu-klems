    ...
    for link in page.find_all("a"):
        ...
        if current_href is not None and not current_href.startswith("#"):
            ...
            if country_code in current_href:
                ...
                file_name = country_code + current_href.split(country_code)[1].strip("?dl=1")
...
def download_eu_klems(
    ...
    country_urls: list[str] - a list of URLs to download
    download_folder: str - the folder to save the files in
    """
    for index, file_name in enumerate(file_names):
        url_of_file = country_urls[index]
        for code in country_codes:
            ...
            if code in file_name:
                ...
                urllib.request.urlretrieve(
                    url_of_file, download_folder / Path(code) / file_name
                )